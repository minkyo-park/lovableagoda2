  ...
  return (
    <>
      ...
      <section className="section-container" aria-label="카드사 프로모션 상세" id="card-promo">
        ...
        {/* Card promo summary table - Desktop */}
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                ...
                <th className="p-3 text-center rounded-tr-lg">할인 적용</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                ["하나카드", "최대 10%", "특정 지역 추가", "월별 갱신", "라운지 추가혜택"],
                ["토스뱅크", "7%", "한국/태국/베트남 10%", "~28/8/31", "장기 이벤트"],
                ["마스터카드", "최대 15%", "-", "월별 갱신", "전 카드 브랜드"],
                ["Visa카드", "6%", "-", "월별 갱신", "USD 결제 권장"],
                ["유니온페이", "15%", "-", "월별 갱신", "코드 UPI15"],
                ...
                <tr key={i} className={i % 2 === 0 ? "bg-card" : "bg-muted/50"}>
                  ...
                  <td className="p-3 border-b border-border text-center">
                    <a href={AFFILIATE_LINK} target="_blank" rel="noopener noreferrer nofollow" className="cta-button text-xs py-1.5 px-3 inline-block">할인 적용</a>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {/* Card promo - Mobile cards */}
        <div className="md:hidden space-y-3 mb-8">
          {[
            { card: "삼성카드", rate: "최대 7%", region: "인니/말레이/이탈리아 10%", period: "~4/30", note: "LINK 혜택" },
            { card: "신한카드", rate: "7%", region: "인니/말레이/이탈리아 10%", period: "~6/30", note: "선착순 자동적용" },
            { card: "우리카드", rate: "7%", region: "특정 지역 10%", period: "~6/30", note: "핀번호 입력" },
            { card: "NH농협", rate: "7%", region: "말레이/스페인/이탈리아 10%", period: "~6/30", note: "신용/체크 모두" },
            { card: "KB국민", rate: "최대 10%", region: "특정 지역 추가", period: "월별 갱신", note: "전용 페이지" },
            { card: "하나카드", rate: "최대 10%", region: "특정 지역 추가", period: "월별 갱신", note: "라운지 추가혜택" },
            { card: "토스뱅크", rate: "7%", region: "한국/태국/베트남 10%", period: "~28/8/31", note: "장기 이벤트" },
            { card: "마스터카드", rate: "최대 15%", region: "-", period: "월별 갱신", note: "전 카드 브랜드" },
            { card: "Visa카드", rate: "6%", region: "-", period: "월별 갱신", note: "USD 결제 권장" },
            { card: "유니온페이", rate: "15%", region: "-", period: "월별 갱신", note: "코드 UPI15" },
            { card: "카카오페이", rate: "11%", region: "전 세계 숙소", period: "월별 갱신", note: "최대 $100" },
          ].map((item, i) => (
            <div key={i} className="bg-card rounded-lg p-4 border border-border">
              <div className="flex justify-between items-center mb-2">
                <span className="font-bold">{item.card}</span>
                <span className="info-badge bg-secondary text-secondary-foreground">{item.rate}</span>
              </div>
              <div className="text-xs text-muted-foreground space-y-1 mb-3">
                <p>특정 지역: {item.region}</p>
                <p>유효기간: {item.period} · {item.note}</p>
              </div>
              <CtaButton text="할인 적용" className="w-full text-xs py-2" />
            </div>
          ))}
        </div>
